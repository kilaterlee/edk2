  ...
  )
{
  ...
  if (!((mEhdr->e_machine == EM_X86_64) || (mEhdr->e_machine == EM_AARCH64) || (mEhdr->e_machine == EM_RISCV64) || (mEhdr->e_machine == EM_LOONGARCH64))) {
    ...
    if (RelShdr->sh_type == SHT_RELA && (*Filter)(SecShdr)) {
      ...
      for (RelIdx = 0; RelIdx < RelShdr->sh_size; RelIdx += (UINT32) RelShdr->sh_entsize) {
        ...
        if (Sym->st_shndx == SHN_UNDEF
          ...
          //
          // Skip error on EM_RISCV64 and EM_LOONGARCH64 becasue no symble name is built
          // from RISC-V and LoongArch toolchain.
          //
          if ((mEhdr->e_machine != EM_RISCV64) && (mEhdr->e_machine != EM_LOONGARCH64)) {
        ...
        if (mEhdr->e_machine == EM_X86_64) {
          ...
          case R_LARCH_GNU_VTENTRY:
            ...
            break;
          default:
            Error (NULL, 0, 3000, "Invalid", "WriteSections64(): %s unsupported ELF EM_LOONGARCH64 relocation 0x%x.", mInImageName, (unsigned) ELF64_R_TYPE(Rel->r_info));
          }
        } else {
          ...
          if (mEhdr->e_machine == EM_X86_64) {
            ...
            switch (ELF_R_TYPE(Rel->r_info)) {
              ...
              case R_LARCH_GNU_VTENTRY:
                ...
                break;
              default:
                  Error (NULL, 0, 3000, "Invalid", "WriteRelocations64(): %s unsupported ELF EM_LOONGARCH64 relocation 0x%x.", mInImageName, (unsigned) ELF64_R_TYPE(Rel->r_info));
            }
          } else {
